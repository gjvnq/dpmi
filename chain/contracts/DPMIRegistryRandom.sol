  ...
  VRFConsumerBase
{
  ...
  mapping(bytes32 => uint128) mintRequestToTokenId;
  ...
  {
    require(LINK.transfer(msg.sender, getLinkMintFee()), INSUFICIENT_FUNDS_FOR_MINTING);
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    ...
    mintRequestToTokenId[requestId] = randomness2uuid(uint128(randomness));
    emit ReadyToMint(to, requestId);
  }
  ...
  function mintFinish(bytes32 requestId) public returns (uint128 tokenId) {
    address to = mintRequestToAddress[requestId];
    string memory uri = mintRequestToUri[requestId];
    tokenId = mintRequestToTokenId[requestId];
    
    super.safeMint(to, tokenId, uri);
    
    delete mintRequestToUri[requestId];
    delete mintRequestToAddress[requestId];
    delete mintRequestToTokenId[requestId];
  }
